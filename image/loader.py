...
from image.meanColors import calcMeanImage, getInputFromImage
...
def imageGenerator( batchSize, imageSize, path ):
	...
	iterations = paths.shape[0] // batchSize
...
def getDataset( batchSize, path):
	...
	return tf.data.Dataset.from_generator(
							lambda : imageGenerator( batchSize, imageSize, path ),
							output_types  = ( tf.float32, tf.float32, tf.float32),
							output_shapes = ( [batchSize, *imageSize, channels],
											  [batchSize, descSize],
